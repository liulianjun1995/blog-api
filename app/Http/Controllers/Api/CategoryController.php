...
class CategoryController extends ApiController
{
    ...
    {
        $list = Category::query()->select(['title', 'router'])->get();
        return $this->success($list);
    }

    public function articles(Request $request, Category $category, $name = '')
    {
        $fields = ['posts.id', 'posts.id as token', 'posts.title', 'posts.abstract', 'posts.category_id', 'posts.cover', 'posts.content', 'posts.recommend', 'posts.top', 'posts.views', 'posts.created_at'];
